...
/**
  ...
  * @retval         none
  */
extern void OLED_show_char(uint8_t row, uint8_t col, uint8_t chr, uint8_t size);
...
/**
  ...
  * @retval         none
  */
extern void OLED_show_string(uint8_t row, uint8_t col, uint8_t *chr, uint8_t size);

/**
 * @brief  显示12×12点阵汉字
 * @param  row: 汉字起始行（0~5，12行/个，64行屏幕可显示5行）
 * @param  col: 汉字起始列（0~10，12列/个，128列屏幕可显示10个）
 * @param  hz_idx: 汉字索引
 */
extern void OLED_show_chinese_16x16(uint8_t row, uint8_t col, uint8_t hz_idx);
...
/**
  ...
  * @retval         none
  */
extern void OLED_printf(uint8_t row, uint8_t col,uint8_t size, const char *fmt,...);
